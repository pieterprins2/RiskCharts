...
portfolios_pfs_bms_wgts_assets <-
  ...
  mutate(JPM = ifelse(portfolio %in% c("VP100", "RD100"), 0, str_sub(portfolio, 7, 8)),
         ...
         MSDEWIN = 1 - JPM) |>
  pivot_longer(cols = c(JPM, MSDEWIN), values_to = "bm_wgt", names_to = "bm") |>
  mutate(asset = ifelse(bm == "MSDEWIN", "Aandelen", "Obligaties"))
...
```{r dates}
dates <- all_prices_local_long %>% ungroup() %>% select(date) %>% arrange(date) |> unique()
...
datum_breaks <- function(start_date, end_date) {
  if(as.Date(end_date) - as.Date(start_date) >= years(6)) {twee_jaar_datums} 
  else if (as.Date(end_date) - as.Date(start_date) >= months(13) &
            as.Date(end_date) - as.Date(start_date) < years(6)) {jaar_datums}
  else if (as.Date(end_date) - as.Date(start_date) > months(4) &
            as.Date(end_date) - as.Date(start_date) < months(13)) {kwartaal_datums}
  else {maand_datums}
}
```
...
asset_verdeling <-
  ...
  select(-pf, -bm) |>
  mutate(bm_wgt = ifelse(asset == "Cash", 0, bm_wgt),
         OU = weight - bm_wgt,
         asset = factor(asset, levels = asset_levels),
...
total_row <-
  asset_weights_tabel |> ungroup() |> summarise(asset = "Totaal", across(where(is.numeric), sum))
...
asset_weights_tabel |>
  ...
  gt() |> 
  gt_pfs_ou() |> 
  tab_header(str_c("Asset categorieën in portefeuilles en benchmarks, ", snapshot_date_format)) |> 
  gt_font_padding_align()
```
...
all_shares_weights_dur_rm_level |> 
  ...
  ggplot() +
    ...
    geom_text(aes(x = portfolio, y = duration+0.5, label = round(duration, 1), fill = NULL), data = all_shares_weights_dur_rm_level) +
    scale_y_continuous(limits = c(0, 11), breaks = 1:10)+
    ggtitle(str_c("Duration portefeuilles en benchmark, ", snapshot_date_format)) +
...
#### Key Rate Durations RM-deel portefeuilles en benchmark
...
all_shares_weights_pds_rm_level %>%
  ...
  ggtitle(str_c("Key Rate Durations RM-deel portefeuilles en benchmark, ", snapshot_date_format)) +
  scale_fill_identity() +
...
if(!str_detect(pad_deel1, "research")) {
...
nice_country_etc_table <- function(item = "country", RDRM = "RD", asof_date = lastdate) {
  ...
  titelportefeuille <-
    case_when(RDRM == "RDRM" ~ "gehele",
              RDRM == "RD" ~ "RD-deel")
  tabel_portfolio_country_region_sector_weights(item = item, RDRM = RDRM) %>%
...
nice_country_etc_table(item = "currency_2", RDRM = "RDRM") %>%
...
nice_country_etc_table(item = "country", RDRM = "RD")
```
...
return_bar_chart <- function(start_date = start_date_attr, end_date = snapshot_date, pf = "vp") {
  pf_vs_bm_returns_kolom(pf = pf) %>%
    filter(date >= start_date,
           date <= end_date) %>%
...
pf_bm_returns_sd_te_12_24 <-
  ...
  mutate(diff = pf_rt - BM_rt,
         te_12 = roll_sd(diff, 12) * sqrt(12),
         te_24 = roll_sd(diff, 24) * sqrt(12),
         pf_sd_12 = roll_sd(pf_rt, 12) * sqrt(12),
         bm_sd_12 = roll_sd(BM_rt, 12) * sqrt(12),
         pf_sd_24 = roll_sd(pf_rt, 24) * sqrt(12),
         bm_sd_24 = roll_sd(BM_rt, 24) * sqrt(12)) %>%
 ...
 attributie_tabel_nice <- 
  ...
  if(difftime(end_date, start_date) |> as.integer() > 31) {
   ...
   if(end_date > start_date) {
     ...
     kbl(str_c("geen maandgevens na ", format(end_date, "%d-%b")))
   }
 }
```
  ...
  if(difftime(end_date, start_date) |> as.integer() > 31) {
    attributie_tabel_nice <- 
     ...
     if(end_date > start_date) {
      ...
      kbl(str_c("geen maandgevens na ", format(lastdate, "%d-%b")))
      }
}
```
...
<!--   } else -->
<!--   kbl(str_c("geen gevens na ", format(lastdate, "%d-%b")))} -->
...
## Waardering
  ...
  - De enige verschillen die kunnen overblijven zijn in de cash standen. De reden hiervoor is dat de cash standen in AIRS na het moment waarop de VOLK bestanden zijn gedraaid, aan het begin van de maand, nog kunnen veranderen.
  - De performance cijfers sluiten dus nagenoeg aan bij de cijfers uit AIRS.
